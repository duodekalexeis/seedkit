...
a. Open the Tor Browser under "Apps" and choose "Open Tor Connection"
to enable networking, and enter your wifi settings (or connect via a
physical ethernet cable, etc.). Then connect to Tor using the most
appropriate option for your situation, and finally start the Tor Browser.

Then, visit the seedkit releases page at:

  https://github.com/duodekalexeis/seedkit/releases/latest

b. Download the seedkit Linux tarball for your architecture, as well
as the checksums.txt and checksums.txt.sig files e.g.

- seedkit_0.3.9_checksums.txt
- seedkit_0.3.9_checksums.txt.sig
- seedkit_Linux_x86_64.tar.gz
- seedkit_Linux_arm64.tar.gz

(download both tar.gz files if you're not sure of your architecture).

Saving to the default `Downloads` folder is fine.


## 3. Verify the seedkit release files

a. In a terminal (Apps > System Tools > Console) import the GPG public
key used to sign the seedkit release files:

```bash
curl -sS https://github.com/duodekalexeis.gpg | gpg --import
```

b. Change to the directory where you downloaded the seedkit release
files (e.g. the default `Downloads` folder):
...
```bash
gpg --verify seedkit_0.3.9_checksums.txt.sig seedkit_0.3.9_checksums.txt
```
...
```bash
grep Linux_x86_64 seedkit_0.3.9_checksums.txt | sha256sum -c
# should output: seedkit_Linux_x86_64.tar.gz: OK

# And/or:
grep Linux_arm64 seedkit_0.3.9_checksums.txt | sha256sum -c
# should output: seedkit_Linux_arm64.tar.gz: OK
```
...
## 4. Restart tails and confirm seedkit is available

Finally, restart tails and verify that seedkit is available in your
